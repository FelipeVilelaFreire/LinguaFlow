...
These rules are written for code-generation agents. Follow them literally.

1. Raw hex/rgb/hsl/px values are allowed only inside token declaration files, such as `web/src/theme/globals.css`, `frontend/src/admin/styles/variables.css`, or `packages/shared-core/src/theme/*`.
2. Components, screens, cards, modals, tabs, forms, and admin apps must use existing tokens only: `var(--color-*)`, `var(--spacing-*)`, `var(--radius-*)`, `var(--font-*)`, `var(--shadow-*)`, shared-core tokens (`COLORS`, `SPACING`, `RADIUS`, `FONT_*`), or `var(--admin-*)` in admin.
3. Never invent one-off tokens inside a component. If a token is missing, add it to `packages/shared-core/src/theme/*` first, mirror it in `web/src/theme/globals.css`, document its purpose, then use it.
4. Never use fallback colors like `var(--color-surface, #fff)` in component CSS. Fallback hex values hide token mistakes.
5. Never use `style={{ color: '#...' }}`, `style={{ background: '#...' }}`, inline px spacing, or literal `white`/`black` for static UI.
6. Brand/status colors must be semantic tokens (`primary`, `success`, `warning`, `error`, `info`). Do not create random decorative color families.
7. If dark mode is requested, surfaces/text/borders must use semantic light/dark tokens. Do not force `var(--color-white)` on common surfaces in dark mode.
8. Before finishing any visual work, search the changed files for `#`, `rgb`, `rgba`, `hsl`, `px`, `white`, and `black`. Any match outside token files needs a reason.
9. If an implementation file needs a value that has no token yet, stop and create/reuse a token first. Do not ship a temporary literal value or a TODO.

Admin rule: admin CSS uses `--admin-*` tokens only. Consumer web tokens, mobile shared-core tokens, and admin tokens should not be mixed unless `ADMIN.md` explicitly says so.
...
### One declared exception: ReactBits

The only place where animations, gradients, particles, glow, and decorative effects are allowed is **public-facing pages** (`/login`, `/register`, `/onboarding`, marketing, 404/500, success/empty states) — and only via the **ReactBits catalog** ([reactbits.dev](https://www.reactbits.dev)).

Rules:
- Live in `web/src/components/decor/` only
- Marked `aria-hidden` (decorative, not content)
- Never imported by operational features, cards, primitives, or screens
- Peer deps (framer-motion, gsap, ogl, three) declared in `package.json` with a `// for ReactBits` comment

Full spec in [`docs/FRONTEND-WEB.md`](./FRONTEND-WEB.md) → "ReactBits — Decorative Components".

If you're not building a public-facing page, ignore ReactBits entirely. Calm rules above are the default.
...
**Tokens**
- [ ] Zero hardcoded colors / px / font-sizes outside token declaration files?
- [ ] Changed files searched for `#`, `rgb`, `rgba`, `hsl`, `px`, `white`, and `black`?
- [ ] Admin styles, if touched, use only `--admin-*` tokens?
- [ ] Shared-core token changes mirrored in `web/src/theme/globals.css` and rebuilt to `dist/`?
