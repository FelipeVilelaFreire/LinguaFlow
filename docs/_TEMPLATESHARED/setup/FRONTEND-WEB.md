...
Stack: **Next.js 16.1 (App Router, Turbopack) + React 19 + CSS Modules + TypeScript 5 + axios + FontAwesome 7 + i18n via TS files**.
**No Tailwind. No CSS-in-JS. No utility libraries.**

Shared-template override:
- web is render-only; all domain logic, services, types, strings, tokens, validation, and mappers live in `packages/shared-core`;
- `web/src/hooks/{domain}/use*.ts` files are thin wrappers only;
- shared features should mirror the mobile tree and any platform difference must be recorded in `docs/PARITY-AUDIT-EXECUTION.md`;
- visible text comes from `STRINGS` imported from `@{{project}}/shared-core`.

### React 19 — defaults

- **Server Components by default.** Every file in `app/` is a Server Component unless you mark it `'use client'`.
- Mark `'use client'` only when you need: hooks (`useState`, `useEffect`, custom hooks), event handlers, browser APIs (`localStorage`, `window`), `Context`, animation libs.
- Screens (`src/screens/*`) are typically `'use client'` because they call data hooks.
- Features (`src/components/features/*`) can be either — prefer Server Components when they only render props.
- Use `use()` for unwrapping promises in Server Components (`const { id } = await params`).
- Use React 19 form actions in Server Components when applicable — they integrate cleanly with Next.js server actions.
...
```typescript
// packages/shared-core/src/constants/strings/en.ts
...
```typescript
// packages/shared-core/src/constants/strings/types.ts
...
const LOCALE = (process.env.NEXT_PUBLIC_LOCALE ?? 'pt-BR') as Locale;
export const STRINGS = LOCALE === 'en' ? EN : LOCALE === 'de-DE' ? DE_DE : PT_BR;
```
...
const aliasMap: Record<string, string> = {
  // FA6 → FA7 renames (FA7 also dropped some legacy FA5 names entirely)
  'pencil-alt': 'pencil',
  'times': 'xmark',
  // semantic aliases
  'edit': 'pencil',
...
## ✨ ReactBits — Decorative Components (scoped exception)

**ReactBits** ([reactbits.dev](https://www.reactbits.dev)) is a copy-paste catalog of animated React components (Aurora, Beams, Threads, ShinyText, DecryptionEffect, StarBorder, TiltedCard, Particles, etc.). It is **not** installed via `npm install reactbits` — you copy the `.tsx` file from the website and install the peer dependencies it declares (typically `framer-motion`, `gsap`, `ogl`, or `three`).

ReactBits is the **only declared exception** to the Calm/Notion-flat rules in `DESIGN.md`. Use it deliberately and within strict scope.

### Where ReactBits IS allowed

- `/login`, `/register`, `/onboarding` — background effects, hero animations
- Future marketing / landing pages
- 404, 500, success/empty states where polish matters
- Auth-adjacent flows where premium feel justifies the weight

### Where ReactBits is FORBIDDEN

- Any operational app screen (`/dashboard`, lists, tables, forms, detail views, settings)
- Any KPI card, table row, status badge, or piece of data UI
- Anywhere `DESIGN.md`'s Calm rules apply (i.e. everywhere not listed above)

### Integration pattern

When scaffolding a page that uses ReactBits:

1. **Stub the component first** as a thin wrapper with the project's brand gradient. Add a `// TODO` comment with the ReactBits URL so the user knows what to copy in:

   ```typescript
   // src/components/decor/AuroraBg.tsx
   // TODO: Replace with ReactBits Aurora — https://www.reactbits.dev/backgrounds/aurora
   export function AuroraBg() {
     return <div className={styles.auroraStub} aria-hidden />;
   }
   ```
   ...
   .auroraStub {
     position: absolute;
     inset: 0;
     background: radial-gradient(circle at 30% 20%, var(--color-primary-light), transparent 60%);
     opacity: 0.5;
     pointer-events: none;
   }
   ```

2. **Add peer deps to `package.json` commented**, so the user knows what to install when copying the real component:
   ...
   ```jsonc
   {
     "dependencies": {
       ...
       // gsap, ogl, three — add when copying components that need them
     }
   }
   ```

3. **Keep all ReactBits-using components in `web/src/components/decor/`** — separate folder, easy to audit, never imported by operational screens.

4. **Always `aria-hidden`** on decorative components — they're visual fluff, not content.

### Component recommendations by brand mood

| Brand mood | Recommended ReactBits |
|------------|-----------------------|
| Premium / financial / dark | Aurora, Beams, ShinyText, DecryptionEffect |
| Tech / SaaS / blue | Threads, Particles, StarBorder |
| Creative / colorful | TiltedCard, Splash, GradientText |
| Minimal / editorial | Skip ReactBits — pure CSS is cleaner |

### Anti-pattern check

If a ReactBits component appears in any of these files, it's a violation:
- `src/components/features/*` (operational features)
- `src/components/cards/*` (data cards)
- `src/components/ui/*` (primitives — keep them framework-free)
- `src/screens/*` (fetch layer should be skeleton, not animated)
...
- [ ] No raw `fetch` / `axios` calls in web UI? (HTTP lives in shared-core services)
- [ ] No `useState` for fetched data inside a component? (only inside shared-core hooks)
...
- [ ] Web/mobile feature tree differences recorded in `docs/PARITY-AUDIT-EXECUTION.md`?
- [ ] New icons registered in `iconMapper.ts`?
- [ ] New routes added to `ROUTES` constant?
- [ ] Domain types in `packages/shared-core/src/types/{domain}/` (not local)?
- [ ] ReactBits components (if any) live in `src/components/decor/` and are imported only by public-facing pages (`/login`, `/register`, `/onboarding`, marketing)?
