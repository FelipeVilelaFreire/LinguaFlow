...
Admin non-negotiables:
- visible text uses `STRINGS.admin.*` in `pt-BR.ts`, `en.ts`, and `de-DE.ts`;
- in this shared-core template, admin strings live in `packages/shared-core/src/constants/strings/` under the `admin` namespace;
- `config.jsx` and `constants.js` store string references, not literal labels;
- entity apps are config-driven and split into `pages/`, `tabs/`, `cards/`, `components/`, `hooks/`, and `utils/`;
- detail pages use tabs for sections and quick info/entity cards for scanability;
- if admin exists, scaffold `apps/business-plan/` as the business model and scenario dashboard.

## Admin Strings Contract

Admin has its own copy namespace: `STRINGS.admin.*`. In each admin app, start from the app namespace and pass references through config:

```jsx
const T = STRINGS.admin.orders;
```

Use this in `config.jsx`, `constants.js`, pages, tabs, cards, modals, toasts, empty states, filters, placeholders, aria labels, and table columns. Do not create local label maps, inline Portuguese/English text, or duplicated status labels. If a visible label is missing, add the key to `packages/shared-core/src/constants/strings/pt-BR.ts`, `en.ts`, and `de-DE.ts` first, rebuild shared-core, then use the `STRINGS.admin.*` reference.
...
└── src/
    ...
    └── admin/
        ...
        │   ├── business-plan/         # Financial model, scenarios, validation backlog
        │   ├── {{entity1}}/
        ...
        │   │   ├── cards/             # entity summary/detail cards
...
export default {
  // Identity
  entity: 'orders',
  title: STRINGS.admin.orders.title,
  singularTitle: STRINGS.admin.orders.singularTitle,
  icon: 'fa-receipt',
  apiEndpoint: API_ENDPOINTS.ORDERS,

  // Quick info cards on detail page header
  quickInfo: (data) => [
    { label: STRINGS.admin.orders.fields.status, value: data.status, type: 'status' },
    { label: STRINGS.admin.orders.fields.total, value: `R$ ${data.total_amount}`, type: 'highlight' },
    { label: STRINGS.admin.orders.fields.created, value: data.created_at, type: 'date' },
  ],
  ...
  detailTabs: [
    {
      key: 'info',
      label: STRINGS.admin.orders.tabs.info,
      icon: 'fa-info-circle',
    ...
    {
      key: 'items',
      label: STRINGS.admin.orders.tabs.items,
      icon: 'fa-list',
  ...
  filters: [
    { key: 'status', label: STRINGS.admin.orders.fields.status, type: 'select', options: ['pending', 'paid', 'cancelled'] },
    { key: 'created_at', label: STRINGS.admin.orders.fields.created, type: 'daterange' },
  ],
};
```
...
export const TABLE_COLUMNS = [
  {
    key: 'id',
    label: STRINGS.admin.orders.fields.id,
    sortable: true,
    width: 80,
  },
  {
    key: 'customer',
    label: STRINGS.admin.orders.fields.customer,
    type: 'custom',
    render: (_v, row) => <span>{row.customer.name}</span>,
  },
  {
    key: 'status',
    label: STRINGS.admin.orders.fields.status,
    type: 'badge',
    badgeMap: {
      pending: { color: 'warning', label: STRINGS.admin.orders.status.pending },
      paid: { color: 'success', label: STRINGS.admin.orders.status.paid },
      cancelled: { color: 'error', label: STRINGS.admin.orders.status.cancelled },
    },
  },
  {
    key: 'total_amount',
    label: STRINGS.admin.orders.fields.total,
    type: 'currency',
  ...
  {
    key: 'created_at',
    label: STRINGS.admin.orders.fields.created,
    format: formatDate,
    sortable: true,
  },
];
...
export const FORM_FIELDS = [
  { name: 'customer_id', label: STRINGS.admin.orders.fields.customer, type: 'select', apiEndpoint: API_ENDPOINTS.USERS,
    valueKey: 'id', labelKey: 'email', required: true },
  { name: 'total_amount', label: STRINGS.admin.orders.fields.totalAmount, type: 'number', required: true, min: 0 },
  { name: 'notes', label: STRINGS.admin.orders.fields.notes, type: 'textarea', rows: 4 },
];

export const INFO_TAB_FIELDS = [
  { name: 'status', label: STRINGS.admin.orders.fields.status, type: 'select',
    options: ['pending', 'paid', 'cancelled'] },
  { name: 'total_amount', label: STRINGS.admin.orders.fields.totalAmount, type: 'number' },
  { name: 'notes', label: STRINGS.admin.orders.fields.notes, type: 'textarea', rows: 6 },
];
```
...
```javascript
{ name: 'icon', label: STRINGS.admin.shared.fields.icon, type: 'custom', component: IconPicker }
```
...
```bash
mkdir -p frontend/src/admin/apps/{{entity}}/{pages,tabs,cards,components,hooks,utils}
```
...
- [ ] New app has `config.jsx`, `constants.js`, `pages/`, `tabs/InfoTab/`, `cards/`?
- [ ] `business-plan/` app exists when admin is scaffolded?
- [ ] All visible labels use `STRINGS.admin.*` in `pt-BR.ts`, `en.ts`, and `de-DE.ts`?
...
- Hardcoded visible labels/copy in JSX, `config.jsx`, or `constants.js` instead of `STRINGS.admin.*`
