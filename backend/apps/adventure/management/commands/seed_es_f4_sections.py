...
class Command(BaseCommand):
    help = "Seed das 6 seções da Fase 4 Espanhol A1 (requer seed_es_full antes)"
    ...
    def handle(self, *args, **options):
        self.stdout.write("\n📦 Seeding seções — ES A1 T1 Fase 4\n")
        ...
        try:
            phase = AdventurePhase.objects.get(chapter=chapter, number=4)
        except AdventurePhase.DoesNotExist:
            raise CommandError(
                "Fase 4 do chapter 'es-a1-t1' não encontrada. Rode 'seed_es_full' primeiro."
            )
        ...
        if existing and not options["reset"]:
            self.stdout.write(
                self.style.WARNING(
                    f"  Fase 4 já tem {existing} seções. Use --reset para recriar."
                )
            )
            return
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\n✅ {created_count} seções criadas para Fase 4 · ES A1 T1\n"
